...
public class BoardsDynamicQuery extends DynamicQueryRequest<List<Board>> {
    /* Gen by Vizend Studio v5.1.0 */

    public void execute(MongoTemplate mongoTemplate) {
        /* Gen by Vizend Studio v5.1.0 */
        DynamicQuery<BoardDoc> query = new MongoDynamicQuery<>(mongoTemplate, getQueryParams(), getOffset(), BoardDoc.class);
        List<BoardDoc> boardDocs = query.findAll();
        setResponse(Optional.ofNullable(boardDocs).map(docs -> BoardDoc.toDomains(docs)).orElse(new ArrayList<>()));
    }
}
